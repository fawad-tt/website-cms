...
export const isAdminOrSiteManager: Access = ({ req: { user } }) => {
  return Boolean(user?.roles?.includes(ROLES.ADMIN) || user?.roles?.includes(ROLES.SITE_MANAGER))
}

export const isAdminOrAPIUser: Access = ({ req: { user } }) => {
  return Boolean(user?.roles?.includes(ROLES.ADMIN) || user?.roles?.includes(ROLES.API_USER))
}
...
export const isAdminOrHasSiteAccess: Access = ({ req: { user } }) => {
  ...
  if (user?.roles?.includes(ROLES.SITE_MANAGER) && user?.sites?.length && user?.sites?.length > 0) {
...
/**
 * For reading the Sites collection itself
 */
